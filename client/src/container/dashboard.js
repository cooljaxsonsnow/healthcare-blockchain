            ...
            <div className="text-center grid grid-cols-1 md:grid-cols-3 md:gap-5 justify-around mx-20 items-center mt-10 mb-10">
              {apps?.map((app, index) => (
                <div key={index} onClick={() => { app.blocked ? <></> : navigate(`${app.link}`) }}>
                  <div
                    // to={`${app.link}`}
                    className="scale-90 bg-white md:scale-100 border-2 rounded-3xl flex flex-col justify-center sm:mt-10 mx-5 md:mx-5 items-center border-primary  p-10 text-xl hover:border-primary-dark cursor-pointer hover:bg-blue-50 transition delay-100 hover:scale-90 md:hover:scale-110"
                  >
                    {/* <img
                  src={app.imgSrc}
